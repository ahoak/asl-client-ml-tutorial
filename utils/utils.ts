  ...
  await Promise.all(
    classes.map(async (n) => {
      ...
      for (let i = 0; i < numItems; i++) {
        ...
        const tensor = Array.from(result.data.slice(i * numComponents, (i + 1) * numComponents)) as number[]
        data.push(tensor);
...
export function trainTestSplit(X: Tensor[], Y: number[][],  options?: {
  testSize?: number,
  randomState?: number}) {
...
export async function getDatasetPartitions(X: number[][], Y: number[], train_split=0.8, val_split=0.1, test_split=0.1, shuffle=true):Promise<[number[][], number[], number[][], number[], number[][], number[]]>{
    ...
    const testX = await tf.data.array(X).skip(trainSize + valSize).take(testSize).toArray()

    const testY = await tf.data.array(YValues).skip(trainSize + valSize).take(testSize).toArray()
    
    return [trainX, trainY, validationX, validationY, testX, testY]
  }

function shuffle(items, seed = null, rand: ()=>number) {
