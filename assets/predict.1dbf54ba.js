import{B as ut,c as Gt,t as on,a as jt,b as q,d as At,e as F,s as Tr,j as Or,l as Pr}from"./utils.a1f7379e.js";const Fr=`<style>\r
  .root {\r
    display: flex;\r
...
`,ni="/asl-client-ml-tutorial/data/testImage.jpg",X=document.createElement("img");X.setAttribute("crossorigin","anonymous");const xn=63;async function jn(c){if(c.instance){const s=new Promise((f,m)=>{X.onload=f,X.onerror=m});X.src=ni,await s;let d=null;try{const f=await c.instance(X,!1);if(f?.jointPositionsFlat&&f.jointPositionsFlat.size!==xn)return An(f.jointPositionsFlat.size??null);if(!f||!f.jointPositionsFlat)return Xt();d=[f]}catch(f){return q(`${f}`)}return{valid:!0,errors:[],data:d}}else return{valid:!1,errors:F("No implementation found!").errors}}const Xt=()=>F(`
It should return an object of the form: <br />
<pre>
return {
        // flattened should look like [x1, y1, z1, x2, y2, z2...., xN, yN, zN]
...
should be:<br />
<pre>[x1, y1, z1, x2, y2, z2...., xN, yN, zN]</pre>
`),Et={valid:!1,validationIssues:Xt().errors,code:Sn};function ri(c){return JSON.stringify(c)}function ii(c){if(c){const s=JSON.parse(c);if(s.transpiledCode)try{s.instance=ct(s.transpiledCode)}catch(d){console.error(d)}return s}return Et}class Cn extends qt{constructor(){super({defaultCode:Sn,defaultState:Et,validate:jn,readonly:!0})}}customElements.define("extract-and-process-joint-positions-step",Cn);const _n="extractAndProcessJointPositions",oi=Object.freeze(Object.defineProperty({__proto__:null,name:_n,Renderer:Cn,defaultState:Et,serialize:ri,deserialize:ii,validateImageSource:X,validate:jn,createIncorrectReturnTypeError:Xt,createIncorrectReturnShapeError:An},Symbol.toStringTag,{value:"Module"}));async function En(c,s){const d={valid:!1,data:null};if(c)try{const f=await Or.exports.loadAsync(c);"model.json"in f.files?(d.data=c,d.valid=!0):d.validationIssues=On().errors}catch(f){d.validationIssues=q(`${f}`).errors,console.error(f)}else d.validationIssues=F("No model data selected!").errors;return d}const Tn=()=>F("No model data selected!"),On=()=>F("The file that was selected does not contain a model.json file!"),Tt={valid:!1,validationIssues:Tn().errors};function si(c){if(c){const s={...c};return c.data&&(s.data=ui(c.data)),JSON.stringify(s)}return null}function ai(c){if(c)try{const s=JSON.parse(c);return s.data&&(s.data=li(s.data)),s}catch(s){console.error("could not deserialize state",s)}return Tt}function li(c){const s=window.atob(c),d=s.length,f=new Uint8Array(d);for(let m=0;m<d;m++)f[m]=s.charCodeAt(m);return f.buffer}function ui(c){let s="";const d=new Uint8Array(c),f=d.byteLength;for(let m=0;m<f;m++)s+=String.fromCharCode(d[m]);return window.btoa(s)}const ci=`<style>\r
	[slot="success"] {\r
		font-weight: bold;\r
	}\r
\r
...
`;class Mn extends ut{#t=null;#n=null;#e=null;#o=null;#r=null;#i=null;#s="webcam";#a=null;#c=!1;#l=null;constructor(){super(fi)}#u=null;get#d(){return this.#u||(this.#u=this.templateRoot.querySelector(".root")),this.#u}#f={valid:!1};get stepState(){return this.#f}set stepState(s){s!==this.#f&&(this.#f=s)}#h=null;get pipelineState(){return this.#h}set pipelineState(s){this.#h=s,this.#i=null;const d=s?.steps.importModel?.data;d&&Pr(d).then(f=>{this.#i=f}),this.#m()}async predict(){if(this.#r&&this.#a&&this.#h&&this.#i){const s=this.#t.imageSource,d=s instanceof HTMLVideoElement;if(!d||s.readyState>=2){const f=await this.#a(this.#i,s,d,jt),m=f?`${f.classification} (${(f.confidence*100).toFixed(2)}%)`:"No Prediction";this.#S(m)}}}connectedCallback(){this.#t=this.#d.querySelector(".video-stream-viewer"),this.#n=this.#d.querySelector(".start-button"),this.#n.onclick=()=>this.#v(),this.#o=this.#d.querySelector(".predictions-output"),this.#e=this.#d.querySelector(".code-editor");const s=this.#d.querySelector(".toggle-code-button");if(this.#e){this.#e.addEventListener("change",()=>this.#m()),this.#e.setAttribute("code",In);let d=!1;s?.addEventListener("click",()=>{d=!d,this.#e.style.display=d?"":"none"})}super.connectedCallback()}#v(){this.#y()}async#y(){this.#c?await this.#p():await this.#b()}async#b(){await this.#p(),this.#c=!0,this.#n.innerText="Stop",await this.#w(),this.#l=(()=>{let s=!1;const d=async()=>{s||(await this.predict(),setTimeout(d,0))};return setTimeout(d,0),()=>s=!0})()}async#p(){this.#c=!1,this.#n.innerText="Start",await this.#g(),this.#l&&(this.#l(),this.#l=null)}async#w(){if(await this.#g(),this.#s==="webcam"){const s={video:!0,width:200,height:200};this.#r=await navigator.mediaDevices.getUserMedia(s),this.#t.stream=this.#r}}async#g(){this.#r&&(this.#r.getTracks().forEach(s=>s.stop()),this.#t.stream=null,this.#r=null)}#S(s){this.#o.innerText=s??""}#m(){if(this.#a=null,this.#e){const s=this.#e.getAttribute("transpiled-code");s&&(this.#a=ct(s,this.pipelineState))}}}customElements.define("run-step",Mn);const Kt={valid:!1,validationIssues:[],code:In};function hi(c){return JSON.stringify(c)}function pi(c){return c?JSON.parse(c):Kt}async function gi(c){try{if(c.instance){const s=c.instance,d=At([1,2,3]);let f=!1;if(d.dispose=()=>f=!0,await s(d),!f)return Nn()}else return F("Missing Implementation!")}catch(s){const d=`${s}`;return q(d)}return{valid:!0,errors:[]}}const Nn=()=>F("Your function did not clean up the tensor that was passed in."),Rn="run",mi=Object.freeze(Object.defineProperty({__proto__:null,name:Rn,Renderer:Mn,defaultState:Kt,serialize:hi,deserialize:pi,validate:gi,createNotCleanedUpError:Nn},Symbol.toStringTag,{value:"Module"})),xt=Object.freeze(Object.defineProperty({__proto__:null,classify:Kr,cleanup:ei,extractAndProcessJointPositions:oi,importModel:di,run:mi},Symbol.toStringTag,{value:"Module"})),U=document.querySelector(".predict-contents"),sn=document.querySelector("step-controller"),lt=U.querySelector(".next-button"),vi=U.querySelector(".predict-container .breadcrumbs"),R=wi(),yi=Ln();async function bi(){U?.classList.add("initializing"),await Dr();const c=await xi();let s=Object.freeze({stepNum:1,name:R[0].getAttribute("name")});sn?.setAttribute("step",`${s}`),lt?.addEventListener("click",()=>{s.stepNum<R.length&&R[s.stepNum-1].stepState.valid&&m(s.stepNum+1)}),U.querySelector(".reset-button")?.addEventListener("click",()=>{const y=R[s.stepNum-1],j=y.getAttribute("name");y.stepState=yi.steps[j]});async function f(y,j){c.steps[y]={...j},await Si(c),s.name===y&&lt?.toggleAttribute("disabled",!c.steps[y].valid);for(const A of R)A.pipelineState=c;ln(R,c)}function m(y){const j=Math.min(y,R.length);s=Object.freeze({stepNum:j,name:R[j-1].getAttribute("name")}),window.location.hash=`#step${s.stepNum}`,sn?.setAttribute("step",`${s.stepNum}`);const A=R[s.stepNum-1].getAttribute("name");R[s.stepNum-1].stepState=c.steps[A],lt&&(lt.toggleAttribute("disabled",!c.steps[A].valid),lt.style.display=y===R.length?"none":""),ln(R,c)}m(an()||1),R.forEach(y=>{y.pipelineState=c;const j=y.getAttribute("name");j&&y.addEventListener("stateChanged",()=>void f(j,y.stepState))}),addEventListener("hashchange",()=>m(an())),U?.classList.add("ready"),U?.classList.remove("initializing")}function an(){return+((window.location.hash??null).replace("#step","")||"1")}function Ln(){return{steps:{importModel:{...Tt},extractAndProcessJointPositions:{...Et},classify:{...Ct},cleanup:{..._t},run:{...Kt}}}}function ln(c,s){for(let d=0;d<c.length;d++){const f=vi?.querySelector(`[step="${d+1}"]`);if(f){const m=c[d].getAttribute("name"),y=s.steps[m]?.valid??!1;f.classList.toggle("valid",y),f.classList.toggle("invalid",!y)}}}function wi(){return[U.querySelector(`[name="${Fn}"]`),U.querySelector(`[name="${_n}"]`),U.querySelector(`[name="${mn}"]`),U.querySelector(`[name="${wn}"]`),U.querySelector(`[name="${Rn}"]`)]}function Si(c){Object.keys(c.steps).forEach(s=>{if(s in xt&&s in c.steps){const d=xt[s].serialize(c.steps[s]);localStorage.setItem(`predict:${s}`,d??"")}})}function xi(){const c=Ln();return Object.keys(c.steps).forEach(s=>{if(s in xt&&s in c.steps){const d=localStorage.getItem(`predict:${s}`);c.steps[s]=xt[s].deserialize(d)}}),c}bi();
