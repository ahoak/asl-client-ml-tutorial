import{c as Vt,t as nn,a as jt,b as ct,d as an,e as N,f as xr,j as jr,l as Ar,g as Cr,J as _r,A as Er,h as Tr,s as Or}from"./utils.83332e14.js";import{B as dt}from"./VideoStreamViewer.c118ebcd.js";const Pr=`<style>\r
  .root {\r
    display: flex;\r
...
`;class Mr extends dt{static get observedAttributes(){return["style","name","step-issues","validating","valid"]}constructor(){super(Pr)}#t=null;#r=null;#e=null;#s=null;#n=null;#i=!1;#a=[];#o=!0;#u=null;get#l(){return this.#u||(this.#u=this.templateRoot.querySelector(".root")),this.#u}attributeChangedCallback(s,d,h){s==="step-issues"&&(this.#a=JSON.parse(this.getAttribute("step-issues")||"[]")),this.#c(s,h??"")}connectedCallback(){this.#t=this.#l.querySelector(".name"),this.#r=this.#l.querySelector(".validate-success"),this.#s=this.#l.querySelector(".validate-progress"),this.#e=this.#l.querySelector(".issue-container"),this.#n=this.#l.querySelector(".issue-display"),this.#i=!0,this.#c(),this.#o=!1}disconnectedCallback(){this.#i=!1}#c(s=null,d){if(this.#i){if(this.#d("style",s)&&(this.#l.style.cssText=d??this.getAttribute("style")??""),this.#d("name",s)&&(this.#t.innerHTML=d??this.getAttribute("name")??""),this.#d("validating",s)){const p=this.hasAttribute("validating");this.#s.style.display=p?"":"none"}this.#d("step-issues",s)&&this.#n.setAttribute("issues",JSON.stringify(this.#a));const h=this.#a.length>0;this.#e.style.display=h?"block":"none",this.#r.style.display=this.hasAttribute("valid")?"block":"none"}}#d(s,d){return this.#o||!d||d===s}}customElements.define("step-container",Mr);const Fr=`<style>
...
`,hn=63,Hr=an(Array.from({length:hn}).map(()=>Math.random()*2-1)).expandDims(0),Ht="C",Wr=()=>an(jt.map(c=>c===Ht?1:0));async function fn(c){try{const s=c.instance,d=Wr(),h=xr({});h.predict=y=>d;const p=await s(h,jt,Hr);if(!p||!p.classification)return N(`
...
`);if(p.classification!==Ht)return N(`classify() returned <b>"${p.classification}"</b>, but it should've returned <b>"${Ht}"</b>`)}catch(s){const d=`${s}`;return d.indexOf("Implement")>=0?N("Your implementation is incomplete"):ct(d)}return{valid:!0,errors:[]}}const pn=()=>N(`
It should return an object of the form: <br />
<pre>
return {
...
`),Ct={valid:!1,validationIssues:pn().errors,code:dn};function Xr(c){return JSON.stringify(c)}function Yr(c){if(c){const s=JSON.parse(c);if(s.transpiledCode)try{s.instance=At(s.transpiledCode)}catch(d){console.error(d)}return s}return Ct}class gn extends cn{constructor(){super({defaultCode:dn,defaultState:Ct,solutionCode:Gr,validate:fn})}}customElements.define("classify-step",gn);const mn="classify",Kr=!0,Zr=Object.freeze(Object.defineProperty({__proto__:null,name:mn,userMutable:Kr,Renderer:gn,defaultState:Ct,serialize:Xr,deserialize:Yr,validate:fn,createIncorrectReturnTypeError:pn,createIncorrectReturnShapeError:qr},Symbol.toStringTag,{value:"Module"})),vn=`
/**
...
`,Qr="/asl-client-ml-tutorial/data/testImage.jpg",Y=document.createElement("img");Y.setAttribute("crossorigin","anonymous");const yn=63;async function bn(c){if(c.instance){const s=new Promise((h,p)=>{Y.onload=h,Y.onerror=p});Y.src=Qr,await s;let d=null;try{const h=await c.instance(Y,!1);if(h?.jointPositionsFlat&&h.jointPositionsFlat.size!==yn)return wn(h.jointPositionsFlat.size??null);if(!h||!h.jointPositionsFlat)return Xt();d=[h]}catch(h){return ct(`${h}`)}return{valid:!0,errors:[],data:d}}else return{valid:!1,errors:N("No implementation found!").errors}}const Xt=()=>N(`
It should return an object of the form: <br />
<pre>
return {
        // flattened should look like [x1, y1, z1, x2, y2, z2...., xN, yN, zN]
...
should be:<br />
<pre>[x1, y1, z1, x2, y2, z2...., xN, yN, zN]</pre>
`),_t={valid:!1,validationIssues:Xt().errors,code:vn};function ti(c){return JSON.stringify(c)}function ei(c){if(c){const s=JSON.parse(c);if(s.transpiledCode)try{s.instance=At(s.transpiledCode)}catch(d){console.error(d)}return s}return _t}const ni=`<style>\r
	[slot="success"] {\r
		font-weight: bold;\r
	}\r
\r
...
</div>`;class Sn extends cn{constructor(){super({defaultCode:vn,defaultState:_t,validate:bn,readonly:!0,showCodeToggleButton:!0,template:ni})}}customElements.define("extract-and-process-joint-positions-step",Sn);const xn="extractAndProcessJointPositions",ri=!1,ii=Object.freeze(Object.defineProperty({__proto__:null,name:xn,userMutable:ri,Renderer:Sn,defaultState:_t,serialize:ti,deserialize:ei,validateImageSource:Y,validate:bn,createIncorrectReturnTypeError:Xt,createIncorrectReturnShapeError:wn},Symbol.toStringTag,{value:"Module"}));async function Wt(c,s){const d={valid:!1,data:null};if(c)try{const h=await jr.exports.loadAsync(c);"model.json"in h.files?(d.data=c,d.valid=!0):d.validationIssues=An().errors}catch(h){d.validationIssues=ct(`${h}`).errors,console.error(h)}else d.validationIssues=N("No model data selected!").errors;return d}const jn=()=>N("No model data selected!"),An=()=>N("The file that was selected does not contain a model.json file!"),Et={valid:!1,validationIssues:jn().errors};function oi(c){if(c){const s={...c};return c.data&&(s.data=li(c.data)),JSON.stringify(s)}return null}function si(c){if(c)try{const s=JSON.parse(c);return s.data&&(s.data=ai(s.data)),s}catch(s){console.error("could not deserialize state",s)}return Et}function ai(c){const s=window.atob(c),d=s.length,h=new Uint8Array(d);for(let p=0;p<d;p++)h[p]=s.charCodeAt(p);return h.buffer}function li(c){let s="";const d=new Uint8Array(c),h=d.byteLength;for(let p=0;p<h;p++)s+=String.fromCharCode(d[p]);return window.btoa(s)}const ui=`<style>\r
	[slot="success"] {\r
		font-weight: bold;\r
	}\r
\r
...
`;class Tn extends dt{#t=null;#r=null;#e=null;#s=null;#n=null;#i=null;#a="webcam";#o=null;#u=!1;#l=null;constructor(){super(hi)}#c=null;get#d(){return this.#c||(this.#c=this.templateRoot.querySelector(".root")),this.#c}#h={valid:!1};get stepState(){return this.#h}set stepState(s){s!==this.#h&&(this.#h=s)}#f=null;get pipelineState(){return this.#f}set pipelineState(s){this.#f=s,this.#i=null;const d=s?.steps.importModel?.data;d&&Tr(d).then(h=>{this.#i=h}),this.#m()}async predict(){if(this.#n&&this.#o&&this.#f&&this.#i){const s=this.#t.imageSource,d=s instanceof HTMLVideoElement;if(!d||s.readyState>=2){const h=await this.#o(this.#i,s,d,jt),p=h?`${h.classification} (${(h.confidence*100).toFixed(2)}%)`:"No Prediction";this.#S(p)}}}connectedCallback(){this.#t=this.#d.querySelector(".video-stream-viewer"),this.#r=this.#d.querySelector(".start-button"),this.#r.onclick=()=>this.#v(),this.#s=this.#d.querySelector(".predictions-output"),this.#e=this.#d.querySelector(".code-editor");const s=this.#d.querySelector(".toggle-code-button");if(this.#e){this.#e.addEventListener("change",()=>this.#m()),this.#e.setAttribute("code",En);let d=!1;s?.addEventListener("click",()=>{d=!d,this.#e.style.display=d?"":"none"})}super.connectedCallback()}#v(){this.#y()}async#y(){this.#u?await this.#p():await this.#b()}async#b(){await this.#p(),this.#u=!0,this.#r.innerText="Stop",await this.#w(),this.#l=(()=>{let s=!1;const d=async()=>{s||(await this.predict(),setTimeout(d,0))};return setTimeout(d,0),()=>s=!0})()}async#p(){this.#u=!1,this.#r.innerText="Start",await this.#g(),this.#l&&(this.#l(),this.#l=null)}async#w(){if(await this.#g(),this.#a==="webcam"){const s={video:!0,width:200,height:200};this.#n=await navigator.mediaDevices.getUserMedia(s),this.#t.stream=this.#n}}async#g(){this.#n&&(this.#n.getTracks().forEach(s=>s.stop()),this.#t.stream=null,this.#n=null)}#S(s){this.#s.innerText=s??""}#m(){if(this.#o=null,this.#e){const s=this.#e.getAttribute("transpiled-code");s&&(this.#o=At(s,this.pipelineState))}}}customElements.define("run-step",Tn);const Yt={valid:!1,validationIssues:[],code:En};function fi(c){return JSON.stringify(c)}function pi(c){return c?JSON.parse(c):Yt}const On="run",gi=!1,mi=Object.freeze(Object.defineProperty({__proto__:null,name:On,userMutable:gi,Renderer:Tn,defaultState:Yt,serialize:fi,deserialize:pi},Symbol.toStringTag,{value:"Module"})),ut=Object.freeze(Object.defineProperty({__proto__:null,classify:Zr,extractAndProcessJointPositions:ii,importModel:di,run:mi},Symbol.toStringTag,{value:"Module"}));Or("predict");const $=document.querySelector(".predict-contents"),rn=document.querySelector("step-controller"),lt=$.querySelector(".next-button"),Gt=$.querySelector(".reset-button"),vi=$.querySelector(".predict-container .breadcrumbs"),L=wi(),yi=Pn();async function bi(){$?.classList.add("initializing"),await $r();const c=await xi();let s=Object.freeze({stepNum:1,name:L[0].getAttribute("name")});rn?.setAttribute("step",`${s}`),lt?.addEventListener("click",()=>{s.stepNum<L.length&&L[s.stepNum-1].stepState.valid&&h(s.stepNum+1)}),Gt?.addEventListener("click",()=>{const p=L[s.stepNum-1],y=p.getAttribute("name");p.stepState=yi.steps[y]});async function d(p,y){c.steps[p]={...y},await Si(c),s.name===p&&lt?.toggleAttribute("disabled",!c.steps[p].valid);for(const A of L)A.pipelineState=c;sn(L,c)}function h(p){const y=Math.min(p,L.length);s=Object.freeze({stepNum:y,name:L[y-1].getAttribute("name")}),window.location.hash=`#step${s.stepNum}`,rn?.setAttribute("step",`${s.stepNum}`);const A=L[s.stepNum-1].getAttribute("name");if(L[s.stepNum-1].stepState=c.steps[A],lt&&(lt.toggleAttribute("disabled",!c.steps[A].valid),lt.style.display=p===L.length?"none":""),Gt){const j=ut[s.name].userMutable;Gt.style.display=j?"":"none"}sn(L,c)}h(on()||1),L.forEach(p=>{p.pipelineState=c;const y=p.getAttribute("name");y&&p.addEventListener("stateChanged",()=>void d(y,p.stepState))}),addEventListener("hashchange",()=>h(on())),$?.classList.add("ready"),$?.classList.remove("initializing")}function on(){return+((window.location.hash??null).replace("#step","")||"1")}function Pn(){return{steps:{importModel:{...Et},extractAndProcessJointPositions:{..._t},classify:{...Ct},run:{...Yt}}}}function sn(c,s){for(let d=0;d<c.length;d++){const h=vi?.querySelector(`[step="${d+1}"]`);if(h){const p=c[d].getAttribute("name"),y=s.steps[p]?.valid??!1;h.classList.toggle("valid",y),h.classList.toggle("invalid",!y)}}}function wi(){return[$.querySelector(`[name="${_n}"]`),$.querySelector(`[name="${xn}"]`),$.querySelector(`[name="${mn}"]`),$.querySelector(`[name="${On}"]`)]}function Si(c){Object.keys(c.steps).forEach(s=>{if(s in ut&&s in c.steps){const d=ut[s].serialize(c.steps[s]);localStorage.setItem(`predict:${s}`,d??"")}})}function xi(){const c=Pn();return Object.keys(c.steps).forEach(s=>{if(s in ut&&s in c.steps){const d=localStorage.getItem(`predict:${s}`);c.steps[s]=ut[s].deserialize(d)}}),c}bi();
