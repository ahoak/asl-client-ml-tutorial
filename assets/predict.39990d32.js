...
`;class Lr extends K{static get observedAttributes(){return["style","sign"]}constructor(){super(Nr)}#t=null;#n=null;#e=!0;#o=null;get#s(){return this.#o||(this.#o=this.templateRoot.querySelector(".root")),this.#o}attributeChangedCallback(s,c,h){this.#r(s,h??"")}connectedCallback(){this.#n=this.#s.querySelector(".sign-name"),this.#t=this.#s.querySelector(".sign-image"),this.#r(),this.#e=!1}#r(s=null,c){if(this.#a(s,"sign")){const h=c??this.getAttribute("sign")??"?";this.#n&&(this.#n.innerHTML=h),this.#t&&(this.#t.src=`/asl-client-ml-tutorial/data/signs/${h.toLowerCase()}.jpg`)}}#a(s,c){return this.#e||!c||c===s}}customElements.define("sign-display",Lr);const $r=`<style>\r
  .root {\r
    display: flex;\r
...
`;class Br extends K{static get observedAttributes(){return["style","name","step-issues","validating","valid"]}constructor(){super($r)}#t=null;#n=null;#e=null;#o=null;#s=null;#r=null;#a=!1;#i=[];#c=!0;#u=null;get#l(){return this.#u||(this.#u=this.templateRoot.querySelector(".root")),this.#u}attributeChangedCallback(s,c,h){s==="step-issues"&&(this.#i=JSON.parse(this.getAttribute("step-issues")||"[]")),this.#d(s,h??"")}connectedCallback(){this.#t=this.#l.querySelector(".name"),this.#n=this.#l.querySelector(".validate-success"),this.#e=this.#l.querySelector(".validate-fail"),this.#s=this.#l.querySelector(".validate-progress"),this.#o=this.#l.querySelector(".issue-container"),this.#r=this.#l.querySelector(".issue-display"),this.#a=!0,this.#d(),this.#c=!1}disconnectedCallback(){this.#a=!1}#d(s=null,c){if(this.#a){if(this.#h("style",s)&&(this.#l.style.cssText=c??this.getAttribute("style")??""),this.#h("name",s)&&(this.#t.innerHTML=c??this.getAttribute("name")??""),this.#h("validating",s)){const p=this.hasAttribute("validating");this.#s.style.display=p?"":"none"}this.#h("step-issues",s)&&this.#r.setAttribute("issues",JSON.stringify(this.#i));const h=this.#i.length>0;this.#o.style.display=h?"block":"none",this.#n.style.display=this.hasAttribute("valid")?"":"none",this.#e.style.display=h&&!this.hasAttribute("valid")?"":"none"}}#h(s,c){return this.#c||!c||c===s}}customElements.define("step-container",Br);const Ur=`<style>
...
`,bn=63,ri=fn(Array.from({length:bn}).map(()=>Math.random()*2-1)).expandDims(0),Kt="C",ii=()=>fn(ht.map(d=>d===Kt?1:0));async function wn(d){try{const s=d.instance,c=ii(),h=Tr({});h.predict=y=>c;const p=await s(h,ht,ri);if(!p||!p.classification)return R(`
...
`);if(p.classification!==Kt)return R(`classify() returned <b>"${p.classification}"</b>, but it should've returned <b>"${Kt}"</b>`)}catch(s){const c=`${s}`;return c.indexOf("Implement")>=0?R("Your implementation is incomplete"):pt(c)}return{valid:!0,errors:[]}}const Sn=()=>R(`
It should return an object of the form: <br />
<pre>
return {
...
`,di="/asl-client-ml-tutorial/data/testImage.jpg",Y=document.createElement("img");Y.setAttribute("crossorigin","anonymous");const Cn=63;async function En(d){if(d.instance){const s=new Promise((h,p)=>{Y.onload=h,Y.onerror=p});Y.src=di,await s;let c=null;try{const h=await d.instance(Y,!1);if(h?.jointPositionsFlat&&h.jointPositionsFlat.size!==Cn)return _n(h.jointPositionsFlat.size??null);if(!h||!h.jointPositionsFlat)return ee();c=[h]}catch(h){return pt(`${h}`)}return{valid:!0,errors:[],data:c}}else return{valid:!1,errors:R("No implementation found!").errors}}const ee=()=>R(`
It should return an object of the form: <br />
<pre>
return {
        // flattened should look like [x1, y1, z1, x2, y2, z2...., xN, yN, zN]
...
should be:<br />
<pre>[x1, y1, z1, x2, y2, z2...., xN, yN, zN]</pre>
`),It={valid:!1,validationIssues:ee().errors,code:An};function hi(d){return JSON.stringify(d)}function fi(d){if(d){const s=JSON.parse(d);if(s.transpiledCode)try{s.instance=Ot(s.transpiledCode)}catch(c){console.error(c)}return s}return It}class Tn extends mn{constructor(){super({defaultCode:An,defaultState:It,validate:En,readonly:!0,instructionsMarkdown:ci})}}customElements.define("extract-and-process-joint-positions-step",Tn);const On="extractAndProcessJointPositions",pi=!1,gi=Object.freeze(Object.defineProperty({__proto__:null,name:On,userMutable:pi,Renderer:Tn,defaultState:It,serialize:hi,deserialize:fi,validateImageSource:Y,validate:En,createIncorrectReturnTypeError:ee,createIncorrectReturnShapeError:_n},Symbol.toStringTag,{value:"Module"}));async function Zt(d,s){const c={valid:!1,data:null};if(d)try{const h=await Or.exports.loadAsync(d);"model.json"in h.files?(c.data=d,c.valid=!0):c.validationIssues=In().errors}catch(h){c.validationIssues=pt(`${h}`).errors,console.error(h)}else c.validationIssues=R("No model data selected!").errors;return c}const Pn=()=>R("No model data selected!"),In=()=>R("The file that was selected does not contain a model.json file!"),Mt={valid:!1,validationIssues:Pn().errors};function mi(d){if(d){const s={...d};return d.data&&(s.data=bi(d.data)),JSON.stringify(s)}return null}function vi(d){if(d)try{const s=JSON.parse(d);return s.data&&(s.data=yi(s.data)),s}catch(s){console.error("could not deserialize state",s)}return Mt}function yi(d){const s=window.atob(d),c=s.length,h=new Uint8Array(c);for(let p=0;p<c;p++)h[p]=s.charCodeAt(p);return h.buffer}function bi(d){let s="";const c=new Uint8Array(d),h=c.byteLength;for(let p=0;p<h;p++)s+=String.fromCharCode(c[p]);return window.btoa(s)}const wi=`<style>\r
  [slot='success'] {\r
    font-weight: bold;\r
  }\r
\r
...
`;class kn extends K{#t=null;#n=null;#e=null;#o=null;#s=null;#r=null;#a=null;#i=null;#c="webcam";#u=null;#l=!1;#d=null;constructor(){super(ji)}#h=null;get#f(){return this.#h||(this.#h=this.templateRoot.querySelector(".root")),this.#h}#p={valid:null};get stepState(){return this.#p}set stepState(s){s!==this.#p&&(this.#p=s)}#g=null;get pipelineState(){return this.#g}set pipelineState(s){this.#g=s,this.#i=null;const c=s?.steps.importModel?.data;c&&Rr(c).then(h=>{this.#i=h}),this.#b()}async predict(){if(this.#r&&this.#u&&this.#g&&this.#i){const s=this.#t.imageSource,c=s instanceof HTMLVideoElement;if(!c||s.readyState>=2){const h=await this.#u(this.#i,s,c,ht),p=h?`${h.classification} (${(h.confidence*100).toFixed(2)}%)`:"No Prediction";this.#j(p)}}}connectedCallback(){if(this.#t=this.#f.querySelector(".video-stream-viewer"),this.#n=this.#f.querySelector(".start-button"),this.#n.onclick=()=>this.#w(),this.#o=this.#f.querySelector(".predictions-output"),this.#e=this.#f.querySelector(".code-editor"),this.#a=this.#f.querySelector(".sign-display-container"),this.#s=this.#f.querySelector(".webcam-selector"),this.#s&&this.#s.addEventListener("change",()=>this.#A()),this.#a){let c="";for(const h of ht)c+=`<sign-display sign="${h}"></sign-display>`;this.#a.innerHTML=c}const s=this.#f.querySelector(".toggle-code-button");if(this.#e){this.#e.addEventListener("change",()=>this.#b()),this.#e.setAttribute("code",Rn);let c=!1;s?.addEventListener("click",()=>{c=!c,this.#e.style.display=c?"":"none"})}super.connectedCallback()}#w(){this.#S()}async#S(){this.#l?await this.#v():await this.#x()}async#x(){await this.#v(),this.#l=!0,this.#n.innerText="Stop",await this.#y(),this.#d=(()=>{let s=!1;const c=async()=>{s||(await this.predict(),setTimeout(c,0))};return setTimeout(c,0),()=>s=!0})()}async#v(){this.#l=!1,this.#n.innerText="Start",await this.#m(),this.#d&&(this.#d(),this.#d=null)}async#y(){if(await this.#m(),this.#c==="webcam"){const s={video:{deviceId:this.#s?.selectedDeviceId??void 0,width:200,height:200}};this.#r=await navigator.mediaDevices.getUserMedia(s),this.#t.stream=this.#r}}async#m(){this.#r&&(this.#r.getTracks().forEach(s=>s.stop()),this.#t.stream=null,this.#r=null)}#j(s){this.#o.innerText=s??""}#b(){if(this.#u=null,this.#e){const s=this.#e.getAttribute("transpiled-code");s&&(this.#u=Ot(s,this.pipelineState))}}async#A(){this.#l&&(await this.#m(),await this.#y())}}customElements.define("run-step",kn);const ne={valid:null,validationIssues:[],code:Rn};function Ai(d){return JSON.stringify(d)}function Ci(d){return d?JSON.parse(d):ne}const Nn="run",Ei=!1,_i=Object.freeze(Object.defineProperty({__proto__:null,name:Nn,userMutable:Ei,Renderer:kn,defaultState:ne,serialize:Ai,deserialize:Ci},Symbol.toStringTag,{value:"Module"})),ft=Object.freeze(Object.defineProperty({__proto__:null,classify:ui,extractAndProcessJointPositions:gi,importModel:xi,run:_i},Symbol.toStringTag,{value:"Module"}));kr("predict");const z=document.querySelector(".predict-contents"),cn=document.querySelector("step-controller"),dt=z.querySelector(".next-button"),Yt=z.querySelector(".reset-button"),Ti=z.querySelector(".predict-container .breadcrumbs"),k=Ii(),Oi=Ln();async function Pi(){z?.classList.add("initializing"),await Xr();const d=await Fi();let s=Object.freeze({stepNum:1,name:k[0].getAttribute("name")});cn?.setAttribute("step",`${s}`),dt?.addEventListener("click",()=>{s.stepNum<k.length&&k[s.stepNum-1].stepState.valid&&h(s.stepNum+1)}),Yt?.addEventListener("click",()=>{const p=k[s.stepNum-1],y=p.getAttribute("name");p.stepState=Oi.steps[y]});async function c(p,y){d.steps[p]={...y},await Mi(d),s.name===p&&dt?.toggleAttribute("disabled",!d.steps[p].valid);for(const A of k)A.pipelineState=d;hn(k,d)}function h(p){const y=Math.min(p,k.length);s=Object.freeze({stepNum:y,name:k[y-1].getAttribute("name")}),window.location.hash=`#step${s.stepNum}`,cn?.setAttribute("step",`${s.stepNum}`);const A=k[s.stepNum-1].getAttribute("name");if(k[s.stepNum-1].stepState=d.steps[A],dt&&(dt.toggleAttribute("disabled",!d.steps[A].valid),dt.style.display=p===k.length?"none":""),Yt){const j=ft[s.name].userMutable;Yt.style.display=j?"":"none"}hn(k,d)}h(dn()||1),k.forEach(p=>{p.pipelineState=d;const y=p.getAttribute("name");y&&p.addEventListener("stateChanged",()=>void c(y,p.stepState))}),addEventListener("hashchange",()=>h(dn())),z?.classList.add("ready"),z?.classList.remove("initializing")}function dn(){return+((window.location.hash??null).replace("#step","")||"1")}function Ln(){return{steps:{importModel:{...Mt},extractAndProcessJointPositions:{...It},classify:{...Pt},run:{...ne}}}}function hn(d,s){for(let c=0;c<d.length;c++){const h=Ti?.querySelector(`[step="${c+1}"]`);if(h){const p=d[c].getAttribute("name"),y=s.steps[p]?.valid;h.classList.toggle("valid",y??!1),h.classList.toggle("invalid",!(y??!0))}}}function Ii(){return[z.querySelector(`[name="${Fn}"]`),z.querySelector(`[name="${On}"]`),z.querySelector(`[name="${jn}"]`),z.querySelector(`[name="${Nn}"]`)]}function Mi(d){Object.keys(d.steps).forEach(s=>{if(s in ft&&s in d.steps){const c=ft[s].serialize(d.steps[s]);localStorage.setItem(`predict:${s}`,c??"")}})}function Fi(){const d=Ln();return Object.keys(d.steps).forEach(s=>{if(s in ft&&s in d.steps){const c=localStorage.getItem(`predict:${s}`);d.steps[s]=ft[s].deserialize(c)}}),d}Pi();
