...
export default class BaseComponent<AttributeNames extends string = any> extends HTMLElement {
  ...
  #initElement(shadow = false, templateContent: string | undefined) {
    ...
    this.templateRoot.innerHTML = this.injectTemplate(templateContent);
  }

  /**
   * Injects the template with variables
   * @param templateContent The templatized content
   * @returns The templatized content with variables replaced
   */
  protected injectTemplate(templateContent: string | undefined): string {
