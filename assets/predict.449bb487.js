import{c as Wt,t as un,a as ht,b as pt,d as fn,e as k,f as Tr,j as Or,l as Pr,g as Ir,J as Mr,A as Fr,h as kr,s as Rr}from"./utils.fd1d5341.js";import{B as K}from"./VideoStreamViewer.3feff95f.js";const Nr=`<style>\r
  .root {\r
    position: relative;\r
	display: grid;\r
...
`;class Lr extends K{static get observedAttributes(){return["style","sign"]}constructor(){super(Nr)}#t=null;#n=null;#e=!0;#o=null;get#r(){return this.#o||(this.#o=this.templateRoot.querySelector(".root")),this.#o}attributeChangedCallback(s,d,h){this.#s(s,h??"")}connectedCallback(){this.#n=this.#r.querySelector(".sign-name"),this.#t=this.#r.querySelector(".sign-image"),this.#s(),this.#e=!1}#s(s=null,d){if(this.#i(s,"sign")){const h=d??this.getAttribute("sign")??"?";this.#n&&(this.#n.innerHTML=h),this.#t&&(this.#t.src=`/asl-client-ml-tutorial/data/signs/${h.toLowerCase()}.jpg`)}}#i(s,d){return this.#e||!d||d===s}}customElements.define("sign-display",Lr);const $r=`<style>\r
  .root {\r
    display: flex;\r
...
`,Qr=["style"];class mn extends K{static get observedAttributes(){return Qr}#t=null;#n=null;#e=null;#o=null;#r=null;#s=null;#i=null;#a=null;#u=null;#c;defaultCode;solutionCode;fullyCommentedSolutionCode;instructionsUrl;instructionsMarkdown;showCodeToggleButton;codeHints;validate;readonly;constructor({defaultCode:s,defaultState:d,solutionCode:h,fullyCommentedSolutionCode:p,readonly:y,showCodeToggleButton:A=!1,hints:j,validate:W,template:Ft=Zr,instructionsUrl:Z,instructionsMarkdown:Q}){super(Ft),this.defaultCode=s,this.fullyCommentedSolutionCode=p??h??null,this.solutionCode=h??p??null,this.instructionsUrl=Z??null,this.instructionsMarkdown=Q??null,this.showCodeToggleButton=A??!1,this.readonly=y??!1,this.codeHints=j??null,this.validate=W,this.#c={...d}}get codeVisible(){return this.#t?.style.display!=="none"}set codeVisible(s){this.#t&&(this.#t.style.display=s?"":"none")}get stepState(){return this.#c}set stepState(s){const d=this.#c;if(s!==this.#c){this.#c=s,d.code!==s.code&&this.#t?.setAttribute("code",s.code||this.defaultCode),this?.toggleAttribute("valid",s.valid),this.#n?.toggleAttribute("valid",s.valid);const h=s.syntaxIssues||[];h.length===0&&h.push(...(s.validationIssues||[]).map(p=>({type:"validation",message:p.detail}))),this.#n?.setAttribute("step-issues",h?JSON.stringify(h||[]):""),this?.toggleAttribute("valid",s.valid),this.dispatchEvent(new CustomEvent("stateChanged",{detail:s}))}}#l=null;get root(){return this.#l||(this.#l=this.templateRoot.querySelector(".root")),this.#l}connectedCallback(){this.#t=this.root.querySelector(".code-editor"),this.#n=this.root.querySelector(".step-container"),this.#e=this.root.querySelector(".solution-editor"),this.#o=this.root.querySelector("#solution-tab"),this.#a=this.root.querySelector(".solve-button"),this.#u=this.root.querySelector(".reset-button"),this.#s=this.root.querySelector(".instructions-image"),this.#i=this.root.querySelector(".instructions-md"),this.#r=this.root.querySelector("#instructions-tab"),this.#s&&(this.#s.style.display=this.instructionsUrl?"":"none",this.#s.src=this.instructionsUrl??"about:blank"),this.#i&&(this.#i.style.display=this.instructionsMarkdown?"":"none",this.#i.innerHTML=this.instructionsMarkdown??"about:blank"),this.#r&&(this.#r.style.display=this.instructionsUrl?"":"none"),this.#u&&(this.#u.style.display=this.readonly?"none":"",this.#u.addEventListener("click",()=>{this.#t?.setAttribute("code",this.defaultCode)})),this.#a&&(this.#a.style.display=!this.readonly&&this.solutionCode?"":"none",!this.readonly&&this.#t&&this.solutionCode&&this.#a.addEventListener("click",()=>{this.#t?.setAttribute("code",this.solutionCode)})),this.#e&&(this.#e.style.display=!this.readonly&&this.fullyCommentedSolutionCode?"":"none",this.fullyCommentedSolutionCode&&this.#e?.setAttribute("code",this.fullyCommentedSolutionCode)),this.#o&&(this.#o.style.display=this.#e?.style.display??"none"),this.#t?.toggleAttribute("readonly",this.readonly),this.#t?.setAttribute("code",this.stepState.code||this.defaultCode),this.#t?.setAttribute("hints",JSON.stringify(this.codeHints||{})),this.#t?.addEventListener("change",s=>{this.#h(s.detail)})}async#h(s){let d,h,p;s?{code:d,transpiledCode:h,issues:p}=s:(d=this.#t?.getAttribute("code")||"",h=this.#t?.getAttribute("transpiled-code")||"",p=JSON.parse(this.#t?.getAttribute("issues")||"[]"));const y=(p??[]).filter(j=>j.type==="error"||j.type==="warning")||[],A={valid:!1,code:d,transpiledCode:h,syntaxIssues:y,data:null,instance:null};if(A.valid=y.length===0,A.validationIssues=[],A.instance=void 0,A.transpiledCode&&A?.syntaxIssues?.length===0)try{A.instance=Ot(A.transpiledCode);const j=await this.validate(A,null);A.valid=j.valid,A.validationIssues.push(...j.errors||[])}catch(j){A.valid=!1,A.validationIssues.push(...pt(`${j}`).errors)}this.stepState=Object.freeze(A)}}const vn=(c,s,d,h=!1)=>`${h?`
...
`,Et="tensor",_t="predictedClassIndex",Tt="predictedClassIndex",Xt="/*\u2728INSERT_HERE\u2728*/",yn=te(Xt,Xt,Xt),ti=te(Et,_t,Tt,void 0,!1),ei=te(Et,_t,Tt,"classifySolution",!0),ni={answer1:{code:Et},answer2:{code:_t},answer3:{code:Tt},solution:{code:vn(Et,_t,Tt)}},bn=63,ri=fn(Array.from({length:bn}).map(()=>Math.random()*2-1)).expandDims(0),Kt="C",ii=()=>fn(ht.map(c=>c===Kt?1:0));async function wn(c){try{const s=c.instance,d=ii(),h=Tr({});h.predict=y=>d;const p=await s(h,ht,ri);if(!p||!p.classification)return k(`
...
`);if(p.classification!==Kt)return k(`classify() returned <b>"${p.classification}"</b>, but it should've returned <b>"${Kt}"</b>`)}catch(s){const d=`${s}`;return d.indexOf("Implement")>=0?k("Your implementation is incomplete"):pt(d)}return{valid:!0,errors:[]}}const Sn=()=>k(`
It should return an object of the form: <br />
<pre>
return {
...
`),Pt={valid:!1,validationIssues:Sn().errors,code:yn};function si(c){return JSON.stringify(c)}function ai(c){if(c){const s=JSON.parse(c);if(s.transpiledCode)try{s.instance=Ot(s.transpiledCode)}catch(d){console.error(d)}return s}return Pt}const li="/asl-client-ml-tutorial/instructions/classify.png";class xn extends mn{constructor(){super({defaultCode:yn,defaultState:Pt,solutionCode:ti,fullyCommentedSolutionCode:ei,validate:wn,hints:ni,instructionsUrl:li,instructionsMarkdown:Dr})}}customElements.define("classify-step",xn);const jn="classify",ui=!0,ci=Object.freeze(Object.defineProperty({__proto__:null,name:jn,userMutable:ui,Renderer:xn,defaultState:Pt,serialize:si,deserialize:ai,validate:wn,createIncorrectReturnTypeError:Sn,createIncorrectReturnShapeError:oi},Symbol.toStringTag,{value:"Module"})),di=`<div>
{{#markdown}}
...
`,hi="/asl-client-ml-tutorial/data/testImage.jpg",Y=document.createElement("img");Y.setAttribute("crossorigin","anonymous");const Cn=63;async function En(c){if(c.instance){const s=new Promise((h,p)=>{Y.onload=h,Y.onerror=p});Y.src=hi,await s;let d=null;try{const h=await c.instance(Y,!1);if(h?.jointPositionsFlat&&h.jointPositionsFlat.size!==Cn)return _n(h.jointPositionsFlat.size??null);if(!h||!h.jointPositionsFlat)return ee();d=[h]}catch(h){return pt(`${h}`)}return{valid:!0,errors:[],data:d}}else return{valid:!1,errors:k("No implementation found!").errors}}const ee=()=>k(`
It should return an object of the form: <br />
<pre>
return {
        // flattened should look like [x1, y1, z1, x2, y2, z2...., xN, yN, zN]
...
should be:<br />
<pre>[x1, y1, z1, x2, y2, z2...., xN, yN, zN]</pre>
`),It={valid:!1,validationIssues:ee().errors,code:An};function fi(c){return JSON.stringify(c)}function pi(c){if(c){const s=JSON.parse(c);if(s.transpiledCode)try{s.instance=Ot(s.transpiledCode)}catch(d){console.error(d)}return s}return It}const gi="/asl-client-ml-tutorial/instructions/extractJointPositions.png";class Tn extends mn{constructor(){super({defaultCode:An,defaultState:It,validate:En,readonly:!0,instructionsUrl:gi,instructionsMarkdown:di})}}customElements.define("extract-and-process-joint-positions-step",Tn);const On="extractAndProcessJointPositions",mi=!1,vi=Object.freeze(Object.defineProperty({__proto__:null,name:On,userMutable:mi,Renderer:Tn,defaultState:It,serialize:fi,deserialize:pi,validateImageSource:Y,validate:En,createIncorrectReturnTypeError:ee,createIncorrectReturnShapeError:_n},Symbol.toStringTag,{value:"Module"}));async function Zt(c,s){const d={valid:!1,data:null};if(c)try{const h=await Or.exports.loadAsync(c);"model.json"in h.files?(d.data=c,d.valid=!0):d.validationIssues=In().errors}catch(h){d.validationIssues=pt(`${h}`).errors,console.error(h)}else d.validationIssues=k("No model data selected!").errors;return d}const Pn=()=>k("No model data selected!"),In=()=>k("The file that was selected does not contain a model.json file!"),Mt={valid:!1,validationIssues:Pn().errors};function yi(c){if(c){const s={...c};return c.data&&(s.data=Si(c.data)),JSON.stringify(s)}return null}function bi(c){if(c)try{const s=JSON.parse(c);return s.data&&(s.data=wi(s.data)),s}catch(s){console.error("could not deserialize state",s)}return Mt}function wi(c){const s=window.atob(c),d=s.length,h=new Uint8Array(d);for(let p=0;p<d;p++)h[p]=s.charCodeAt(p);return h.buffer}function Si(c){let s="";const d=new Uint8Array(c),h=d.byteLength;for(let p=0;p<h;p++)s+=String.fromCharCode(d[p]);return window.btoa(s)}const xi=`<style>\r
  [slot='success'] {\r
    font-weight: bold;\r
  }\r
\r
...
`,Mn="import-model-step";class Qt extends K{static get observedAttributes(){return["style"]}constructor(){super(xi)}#t=null;#n=null;#e=null;#o=null;#r=null;#s={...Mt};get stepState(){return this.#s}set stepState(s){s!==this.#s&&(this.#s=s,this?.toggleAttribute("valid",s.valid),this.#r?.toggleAttribute("valid",s.valid),this.#r?.setAttribute("step-issues",s.validationIssues?JSON.stringify((s.validationIssues||[]).map(d=>({type:"validation",message:d.detail}))):""),this.#o&&(this.#o.style.display=s.valid?"":"none"),this.#e&&(this.#e.style.display=s.valid?"none":""),this.dispatchEvent(new CustomEvent("stateChanged",{detail:s})))}#i=null;get#a(){return this.#i||(this.#i=this.templateRoot.querySelector(".root")),this.#i}connectedCallback(){this.#r=this.#a.querySelector(".step-container"),this.#e=this.#a.querySelector(".model-import-container"),this.#n=this.#a.querySelector(".localstorage-import-button"),this.#n?.addEventListener("click",()=>void this.#u()),this.#o=this.#a.querySelector(".reset-button"),this.#o?.addEventListener("click",()=>{this.stepState={valid:!1,validationIssues:k("No model data selected!").errors,data:null}}),this.#t=this.#a.querySelector(".model-import-file"),this.#t?.addEventListener("change",()=>void this.#c()),this.#c()}async#u(){const s={valid:!1,data:null};try{const d=await Pr(Ir),h=new Mr,y=await d.save(new Fr);Object.keys(y.data).forEach((j,W)=>{W===0?h.file(j,JSON.stringify(y.data[j])):h.file(j,y.data[j])});const A=await h.generateAsync({type:"arraybuffer"});Object.assign(s,await Zt(A))}catch(d){s.valid=!1,s.validationIssues=k(`Could not load model from "Build" step!: ${d}`).errors,console.error(d)}this.stepState=Object.freeze(s)}async#c(){const s={valid:!1,data:null},d=this.#t?.files;if(d&&d.length>0){const h=d[0];try{const p=await h.arrayBuffer();Object.assign(s,await Zt(p))}catch(p){s.valid=!1,s.validationIssues=pt(`Could not load zip file, ${p}`).errors,console.error(p)}this.#t.value=""}else s.valid=!1,s.validationIssues=k("No model data selected!").errors;this.stepState=Object.freeze(s)}}customElements.define(Mn,Qt);const ji=!0,Fn="importModel",Ai=Object.freeze(Object.defineProperty({__proto__:null,userMutable:ji,name:Fn,Renderer:Qt,elementName:Mn,ImportModelStep:Qt,validate:Zt,createNoModelSelectedError:Pn,createInvalidModelFileError:In,defaultState:Mt,serialize:yi,deserialize:bi},Symbol.toStringTag,{value:"Module"})),kn=`
/**
...
`;class Rn extends K{#t=null;#n=null;#e=null;#o=null;#r=null;#s=null;#i=null;#a="webcam";#u=null;#c=!1;#l=null;constructor(){super(Ci)}#h=null;get#d(){return this.#h||(this.#h=this.templateRoot.querySelector(".root")),this.#h}#f={valid:!1};get stepState(){return this.#f}set stepState(s){s!==this.#f&&(this.#f=s)}#p=null;get pipelineState(){return this.#p}set pipelineState(s){this.#p=s,this.#i=null;const d=s?.steps.importModel?.data;d&&kr(d).then(h=>{this.#i=h}),this.#v()}async predict(){if(this.#r&&this.#u&&this.#p&&this.#i){const s=this.#t.imageSource,d=s instanceof HTMLVideoElement;if(!d||s.readyState>=2){const h=await this.#u(this.#i,s,d,ht),p=h?`${h.classification} (${(h.confidence*100).toFixed(2)}%)`:"No Prediction";this.#x(p)}}}connectedCallback(){if(this.#t=this.#d.querySelector(".video-stream-viewer"),this.#n=this.#d.querySelector(".start-button"),this.#n.onclick=()=>this.#y(),this.#o=this.#d.querySelector(".predictions-output"),this.#e=this.#d.querySelector(".code-editor"),this.#s=this.#d.querySelector(".sign-display-container"),this.#s){let d="";for(const h of ht)d+=`<sign-display sign="${h}"></sign-display>`;this.#s.innerHTML=d}const s=this.#d.querySelector(".toggle-code-button");if(this.#e){this.#e.addEventListener("change",()=>this.#v()),this.#e.setAttribute("code",kn);let d=!1;s?.addEventListener("click",()=>{d=!d,this.#e.style.display=d?"":"none"})}super.connectedCallback()}#y(){this.#b()}async#b(){this.#c?await this.#g():await this.#w()}async#w(){await this.#g(),this.#c=!0,this.#n.innerText="Stop",await this.#S(),this.#l=(()=>{let s=!1;const d=async()=>{s||(await this.predict(),setTimeout(d,0))};return setTimeout(d,0),()=>s=!0})()}async#g(){this.#c=!1,this.#n.innerText="Start",await this.#m(),this.#l&&(this.#l(),this.#l=null)}async#S(){if(await this.#m(),this.#a==="webcam"){const s={video:!0,width:200,height:200};this.#r=await navigator.mediaDevices.getUserMedia(s),this.#t.stream=this.#r}}async#m(){this.#r&&(this.#r.getTracks().forEach(s=>s.stop()),this.#t.stream=null,this.#r=null)}#x(s){this.#o.innerText=s??""}#v(){if(this.#u=null,this.#e){const s=this.#e.getAttribute("transpiled-code");s&&(this.#u=Ot(s,this.pipelineState))}}}customElements.define("run-step",Rn);const ne={valid:!1,validationIssues:[],code:kn};function Ei(c){return JSON.stringify(c)}function _i(c){return c?JSON.parse(c):ne}const Nn="run",Ti=!1,Oi=Object.freeze(Object.defineProperty({__proto__:null,name:Nn,userMutable:Ti,Renderer:Rn,defaultState:ne,serialize:Ei,deserialize:_i},Symbol.toStringTag,{value:"Module"})),ft=Object.freeze(Object.defineProperty({__proto__:null,classify:ci,extractAndProcessJointPositions:vi,importModel:Ai,run:Oi},Symbol.toStringTag,{value:"Module"}));Rr("predict");const z=document.querySelector(".predict-contents"),cn=document.querySelector("step-controller"),dt=z.querySelector(".next-button"),Yt=z.querySelector(".reset-button"),Pi=z.querySelector(".predict-container .breadcrumbs"),R=Fi(),Ii=Ln();async function Mi(){z?.classList.add("initializing"),await Yr();const c=await Ri();let s=Object.freeze({stepNum:1,name:R[0].getAttribute("name")});cn?.setAttribute("step",`${s}`),dt?.addEventListener("click",()=>{s.stepNum<R.length&&R[s.stepNum-1].stepState.valid&&h(s.stepNum+1)}),Yt?.addEventListener("click",()=>{const p=R[s.stepNum-1],y=p.getAttribute("name");p.stepState=Ii.steps[y]});async function d(p,y){c.steps[p]={...y},await ki(c),s.name===p&&dt?.toggleAttribute("disabled",!c.steps[p].valid);for(const A of R)A.pipelineState=c;hn(R,c)}function h(p){const y=Math.min(p,R.length);s=Object.freeze({stepNum:y,name:R[y-1].getAttribute("name")}),window.location.hash=`#step${s.stepNum}`,cn?.setAttribute("step",`${s.stepNum}`);const A=R[s.stepNum-1].getAttribute("name");if(R[s.stepNum-1].stepState=c.steps[A],dt&&(dt.toggleAttribute("disabled",!c.steps[A].valid),dt.style.display=p===R.length?"none":""),Yt){const j=ft[s.name].userMutable;Yt.style.display=j?"":"none"}hn(R,c)}h(dn()||1),R.forEach(p=>{p.pipelineState=c;const y=p.getAttribute("name");y&&p.addEventListener("stateChanged",()=>void d(y,p.stepState))}),addEventListener("hashchange",()=>h(dn())),z?.classList.add("ready"),z?.classList.remove("initializing")}function dn(){return+((window.location.hash??null).replace("#step","")||"1")}function Ln(){return{steps:{importModel:{...Mt},extractAndProcessJointPositions:{...It},classify:{...Pt},run:{...ne}}}}function hn(c,s){for(let d=0;d<c.length;d++){const h=Pi?.querySelector(`[step="${d+1}"]`);if(h){const p=c[d].getAttribute("name"),y=s.steps[p]?.valid??!1;h.classList.toggle("valid",y),h.classList.toggle("invalid",!y)}}}function Fi(){return[z.querySelector(`[name="${Fn}"]`),z.querySelector(`[name="${On}"]`),z.querySelector(`[name="${jn}"]`),z.querySelector(`[name="${Nn}"]`)]}function ki(c){Object.keys(c.steps).forEach(s=>{if(s in ft&&s in c.steps){const d=ft[s].serialize(c.steps[s]);localStorage.setItem(`predict:${s}`,d??"")}})}function Ri(){const c=Ln();return Object.keys(c.steps).forEach(s=>{if(s in ft&&s in c.steps){const d=localStorage.getItem(`predict:${s}`);c.steps[s]=ft[s].deserialize(d)}}),c}Mi();
