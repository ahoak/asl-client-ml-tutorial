...
const trainingContainerElement: HTMLElement | null = document.querySelector(".training-feedback-container");
...
const loadActionButton = document.querySelector(
  ".load-data-button"
) as HTMLButtonElement;
let x_train : number[][]
let y_train: number[]
...
function validateBackend() {
  ...
  if (backendInUse) {
    ...
    solutionFeedbackElement ? solutionFeedbackElement.innerHTML = `Nice work! You are using ${backendInUse}.` : null
  } else {
    solutionFeedbackElement ?  solutionFeedbackElement.innerHTML =
      "Hmm no backend detected. Please check solution." : null
...
function handleBeforeDataLoadingStyles() {
  ...
  loadingElement ? 
  loadingElement.style.visibility = "visible" : null
}
...
function onBatchEnd(epoch: number, batch: number, logs: {loss: number, acc: number}) {
  if (!initTime) {
    initTime = true;
  ...
  trainingStatusElement ? trainingStatusElement.innerHTML = `
...
function onEpochEnd(epoch: number) {
  ...
  timeElement ? timeElement.innerHTML = `${time} ${
